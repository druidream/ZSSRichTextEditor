...
zss_editor.templateVariableClassName = 'customplaceholder';

zss_editor.templateVariableFocusedClassName = 'customplaceholder_focus';
...
zss_editor.init = function () {
    $(document).ready(function () {
        ...
        var variableObjects = $('.customplaceholder');
        variableObjects.on('touchend', function () {
            console.log('placeholder touchend')
        });

        variableObjects.on('click', function () {
            console.log('placeholder click')
        });

        variableObjects.on('focus', function () {
            console.log('placeholder focus')
        });

        variableObjects.on('blur', function () {
    ...
    $('#zss_editor_content').on('touchend', function (e) {
       console.log('touchend');
        ...
        zss_editor.restorerange(); // backup-restore mechanism to avoid select part of the Variable, here is the restore step
    });

    $(document).on('selectstart', function (e) {
        console.log('on selectstart')
    });

    $(document).on('selectionchange', function (e) {

        var sel = window.getSelection();
        if ($(sel.anchorNode.parentNode).hasClass(zss_editor.templateVariableClassName) !== $(sel.focusNode.parentNode).hasClass(zss_editor.templateVariableClassName)) {
            return false;
        }
        zss_editor.backuprange(); // backup-restore mechanism to avoid select part of the Variable, here is the backup step
        zss_editor.calculateEditorHeightWithCaretPosition();
        zss_editor.setScrollPosition();
        zss_editor.enabledEditingItems(e);
        var range = sel.getRangeAt(0);
        console.log('selectionchange. sel.anchorNode: ' + sel.anchorNode.parentNode.nodeName
            + ', sel.focusNode: ' + sel.focusNode.parentNode.nodeName);

        // console.log("$(sel.anchorNode.parentNode): " + sel.anchorNode.parentNode.nodeName);
        // console.log("$(sel.focusNode.parentNode): " + sel.focusNode.parentNode.nodeName);
        // console.log("hasClass: " + $(sel.anchorNode.parentNode).hasClass(zss_editor.templateVariableClassName));
        // console.log("hasClass: " + $(sel.focusNode.parentNode).hasClass(zss_editor.templateVariableClassName));

        zss_editor.focusVariable();
        if ($(node).hasClass(zss_editor.templateVariableClassName)) {
            var node = zss_editor.getSelectedNode();
            var newRange = document.createRange();
            newRange.setStartBefore(node.childNodes[0]);
            newRange.setEndAfter(node.childNodes[0]);
            var sel = window.getSelection();
            sel.removeAllRanges();
...
//    $('.custom-placeholder').on('click', function(){
//                                console.log('click1')
//                                })
//
//    $('.custom-placeholder').click(function(){
//        console.log('click')
//        $(this).focus()
//        // $(this).addClass('is-focused')
//    })
//    $('.custom-placeholder').focus(function(){
//        console.log('focus')
//    })
//    $('.custom-placeholder').blur(function(){
//        console.log('blur')
//        // $(this).removeClass('is-focused')
//    })
//
//    $('#custom-placeholder').on('input',(e)=>{
//        console.log('placeholder change')
//        $(this).focus()
//        // $(this).addClass('is-focused')
//    })
//
//    $('#zss_editor_content').on('input',function(event){
//        console.log('editor change')
//        var root = $('#edo_container')
...
//    })
    ...
    $('#zss_editor_content').on('keydown', function (e) {
        ...
        var divNode = $(window.getSelection().focusNode.parentNode);
        console.log(divNode);

        // return key monitor
        // should blur if 'Return' key is pressed while editing variable
        if (e.keyCode === 13 && divNode.hasClass(zss_editor.templateVariableClassName)) {
            // window.getSelection().modify("move", "forward", "paragraph");
            // var nextSibling = window.getSelection().focusNode.parentNode.nextSibling;
            console.log("nextSibling: " + window.getSelection().focusNode.parentNode.nextSibling);
            var nextSibling = zss_editor.getSelectedNode().nextSibling;
            if (!nextSibling) {
                var emptyTextNode = document.createElement("span");
                emptyTextNode.innerHTML = "\u200b"; // zero width space. https://en.wikipedia.org/wiki/Zero-width_space
                divNode.after(emptyTextNode);
                nextSibling = emptyTextNode
            }
            var range = document.createRange();
            range.setStartAfter(nextSibling.childNodes[0]);
            range.setEndAfter(nextSibling.childNodes[0]);
            var sel = window.getSelection();
            sel.removeAllRanges();
            sel.addRange(range);

            return false; // cancel the default Return behavior, which otherwise would split the Variable and place the right part on a new line
        }
        ...
        if (e.keyCode === 8) {
            ...
            if (focusNode.textContent.charCodeAt(0) === 8203
                && focusNode.textContent.length === 1
                && $(focusNode.parentNode.previousSibling).hasClass(zss_editor.templateVariableClassName)) { // \u200B
                window.getSelection().focusNode.parentNode.previousSibling.remove();
            }
        }
    });
};//end
...
//        console.log("s.nodeName: " + s.nodeName.toLowerCase())
...
zss_editor.focusVariable = function () {
    $('.customplaceholder').removeClass(zss_editor.templateVariableFocusedClassName);
//    var node = window.getSelection().anchorNode.parentNode
    var node = zss_editor.getSelectedNode();
    if ($(node).hasClass(zss_editor.templateVariableClassName)) {
        $(node).addClass(zss_editor.templateVariableFocusedClassName)
    }
};

zss_editor.blurVariable = function () {
